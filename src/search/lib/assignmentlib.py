...
class AssignmentClass():
    ...
    def min_free_slot(self):
        res = self.s.filter("term", status=STATUS_FREE).aggs.metric("assign_slot", "min", field=SLOT_NO).execute()
        if res:
    ...
    def max_occupied_slot_plus_one(self):
        try:
            res = self.s.filter("term", status=STATUS_OCCUPIED).aggs.metric("assign_slot", "max", field=SLOT_NO).execute()
    ...
    def assign_slot(self):
        ...
        if assigned_slot:
           ...
           assigned_slot = self.max_occupied_slot_plus_one()
           return assigned_slot
